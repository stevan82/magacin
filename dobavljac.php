...
<?php 
include "classes/dobavljaci.php";
include "header.php";
include "connect.php";

//update
if (ISSET($_POST['izmena'])){	
    $dobavljac=new Dobavljaci($conn);
	$dobavljac->naziv=htmlspecialchars($_POST['naziv']);
	$dobavljac->id=htmlspecialchars($_POST['id']);
	$dobavljac->update($dobavljac->naziv,$dobavljac->id);
}
//delete
if (ISSET($_GET['delete_id'])){	
	$dobavljac=new Dobavljaci($conn);
	$dobavljac->id=htmlspecialchars($_GET['delete_id']);
	$dobavljac->delete($dobavljac->id);
}
...
if (ISSET($_POST['unos'])){	
	$dobavljac=new Dobavljaci($conn);
	$dobavljac->naziv=htmlspecialchars($_POST['naziv']);
	$dobavljac->insert($dobavljac->naziv);
}
?>
  <div class="w3-panel">
    <div class="w3-row-padding" style="margin:0 -16px">
      <div class="w3-third">
		...
		<?php if (ISSET($_GET['id'])) echo "<input type='submit' value='Izmeni' name='izmena'><a href='dobavljac.php'><input type='button' value='Prekid'></a>";?>
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
	  $naziv=$row['naziv'];
    echo "<tr><td style='text-align:center'>" . $i. "</td><td><a href='dobavljac.php?id=".$row["id"]."&naziv=".$naziv."'>" . $row["naziv"]. "</td><td style='text-align:center'><a href='dobavljac.php?delete_id=".$row["id"]."'><i style='color:red' class='fa fa-times-circle'></i></td></tr>";
  $i++;
  }
} else {
  echo "0 rezultata";
