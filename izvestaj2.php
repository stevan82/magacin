  ...
  <div class="w3-panel">
    <div class="w3-row-padding" style="margin:0 -16px">
      <div class="w3-third">
		...
		?>
		<tr><td>Roba:</td><td><select name='roba'>
	<?php	if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
	if ($row['id']==$_GET['radnik_id']) {$selected='selected';}
	else $selected='';
    echo "Roba:<option value=" . $row["id"]. " ".$selected.">" . $row["naziv"]. "</option> ";
	}
	}
?>
		</select><br>
...
WHERE mgc_vrsta_robe.id=".$_POST['roba']." AND YEAR(datum_izdavanja)=".$_POST['godina'].
" UNION
...
WHERE mgc_vrsta_robe.id=".$_POST['roba']." AND YEAR(datum_vracanja)=".$_POST['godina'].
...
echo "</table>";
echo "<a href='izvestaj2print.php?roba=".$_POST['roba']."&godina=".$_POST['godina']."' target='_blank'><img src='images/print.png' style='width:100px;height:100px'></a>";
$conn->close();
