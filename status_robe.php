...
if (ISSET($_POST['izmena'])){	
	$status=new StatusRobe($conn);
	$status->naziv=htmlspecialchars($_POST['naziv']);	
	$status->id=htmlspecialchars($_POST['id']);
	$status->update($status->naziv,$status->id);
}

//delete
if (ISSET($_GET['delete_id'])){	
	$status=new StatusRobe($conn);
	$status->id=htmlspecialchars($_GET['delete_id']);
  ...
  <div class="w3-panel">
    <div class="w3-row-padding" style="margin:0 -16px">
      <div class="w3-third">
		...
		<?php if (ISSET($_GET['id'])) echo "<input type='submit' value='Izmeni' name='izmena'><a href='status_robe.php'><input type='button' value='Prekid'></a>";?>
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
	  $naziv=$row['naziv'];
    echo "<tr><td style='text-align:center'>" . $i. "</td><td><a href='status_robe.php?id=".$row["id"]."&naziv=".$naziv."'>" . $row["naziv"]. "</td><td style='text-align:center'><a href='status_robe.php?delete_id=".$row["id"]."'><i style='color:red' class='fa fa-times-circle'></i></td></tr>";
  $i++;
  }
} else {
  echo "0 rezultata";
